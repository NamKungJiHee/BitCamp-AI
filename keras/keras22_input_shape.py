...
model = Sequential()
#model.add(Dense(10, input_dim=3))   # (100,3)  -> (N,3)      #이렇게 했더니 다차원에서 문제가 있움! 그래서 대신 input_shape를 해줌!
model.add(Dense(10, input_shape=(3,)))     # 2차원에서는 x기 (100,3)이므로 3으로 해줌!    # if x.reshape(1,10,10,3) 이거라면 맨 앞이 행이 된다! 여기서 행(맨 앞에있는애)은 무시하고 나머지만 써준다! 즉 10,10,3을 써준다!
