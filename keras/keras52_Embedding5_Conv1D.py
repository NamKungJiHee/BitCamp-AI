...
from tensorflow.keras.layers import Dense,LSTM,Embedding, Flatten  # Embedding: 좌표로 change
...
model.add(Embedding(input_dim = 28, output_dim= 10, input_length=5)) # Embedding: 원핫인코딩안한것을 벡터화해주는것 / Embedding에서만 input_dim이 output이 아니다. 통상적으로는 그 자리가 output 갯수 / output_dim이 출력 갯수
#model.add(Embedding(27,10,input_length=5)) #(N,5,10) --> (N,50)
#model.add(Embedding(28,10))  # #(N,N,10)
#model.add(LSTM(32))
model.add(Conv1D(32,2))  # kernel size = 2,2
model.add(Flatten())
...
model.fit(pad_x, labels, epochs = 100, batch_size=10)   # labels = y
