...
#2. 모델구성
model = Sequential()
model.add(LSTM(10, return_sequences = True, input_shape = (3,1)))  # (N,3,1) -> (N,3,10)  # return_sequences=True를 사용하면 3차원으로 change
...
start= time.time()
model.fit(x,y,epochs=10000,callbacks=[es], batch_size=1)  
end= time.time()- start
