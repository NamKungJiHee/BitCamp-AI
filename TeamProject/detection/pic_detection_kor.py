...
img = cv2.resize(img, None, fx=0.4, fy=0.4) #이미지 크기 재설정 fx: 현재 이미지 가로의 몇배, fy: 현재 이미지 세로의 몇배,None: img_scaled resize크기는 지정안하겠다. 
height, width, channels = img.shape


# Detecting objects
blob = cv2.dnn.blobFromImage(img, 0.00392, (416, 416), (0, 0, 0), True, crop=False) #crop=False: 크기를 조정한 후 이미지를 자를지 말지 여부
#네트워크에 넣어줄 이미지를 blob로 만들어준다.  # blob: scalefactor(선택적으로 특정 요소만큼 이미지 크기를 늘리거나 줄일 수 있음), size(신경망이 예상하는 이미지의 크기)
# True: SwapRB로 첫번째와 마지막 채널을 교환하는 것
# (0, 0, 0) : mean - 입력 영상 각 채널에서 뺄 평균값.
