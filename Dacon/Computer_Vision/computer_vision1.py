...
path = 'D:\\_data\\dacon\\computer\\train\\*' # 모든 이미지를 불러오는 함수
...
print(training_images.shape) # Create a dataset #(50000, 32, 32, 3)
print(training_labels.shape) # Create a dataset #(50000, 1)

#create test dataset

path = 'D:\\_data\\dacon\\computer\\test\\*'
...
training_image_aug = image_generator.flow(training_image_aug, np.zeros(augment_size), batch_size=augment_size, shuffle=False, seed = seeds).next()[0]
...
sample_submission = pd.read_csv("./data/dacon/computer/sample_submission.csv")
