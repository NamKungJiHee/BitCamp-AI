...
#2. 모델링
model = XGBRegressor(n_estimators = 4000, learning_rate =0.25, n_jobs = -1, 
                     max_depth = 13,        
                     min_child_weight = 1, 
                     subsample = 1,        
                     colsample_bytree = 1, 
...
parameters = [
    {'dt__max_depth' : [6, 8, 10], 'dt__min_samples_leaf' : [3, 5, 7, 9]},
...
# parameters = [{'n_estimators' : [100, 200], 'max_depth' : [4, 6, 8, 10], 'min_samples_leaf' : [3, 5, 7, 10]},
#                {'min_samples_leaf' : [3, 5, 7, 10], 'min_samples_split' : [2, 3, 5, 10, 11]},
#              {'min_samples_split' : [2, 3, 5, 10, 11],}] #'max_depth' : [6,8,10,12]}]                  
# model = RandomizedSearchCV(RandomForestRegressor(), parameters, cv = kfold, verbose=1, refit=True, n_jobs=-1)  # RandomizedSearchCV  GridSearchCV

#model = KNeighborsRegressor(n_neighbors=1000, leaf_size=100,n_jobs=-1)
#model = DecisionTreeRegressor(splitter="best", max_depth=1000, min_samples_split=10, min_samples_leaf=30)
